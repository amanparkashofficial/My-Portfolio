    ...
    <header
      className={`sticky top-0 z-50 shadow-md transition-colors duration-300 ${
        darkMode
          ...
          : "bg-gradient-to-r from-white via-blue-100 to-blue-200 text-blue-900"
      }`}
    >
      <div className="max-w-7xl mx-auto px-6 py-2 flex justify-between items-center">
        <h1
          className={`text-lg md:text-xl font-bold cursor-pointer select-none transition-colors duration-300 ${
            darkMode ? "hover:text-indigo-400" : "hover:text-blue-700"
          }`}
          onClick={() => scrollToSection("hero")}
          tabIndex={0}
          onKeyDown={(e) =>
            ["Enter", " "].includes(e.key) && scrollToSection("hero")
          }
        ...
        {/* Desktop Menu */}
        <nav
          className={`hidden md:flex space-x-2 text-xs md:text-sm font-semibold ${
        ...
        <div className="flex items-center space-x-2">
          <button
            ...
            className={`p-1.5 rounded-full transition-colors duration-300 focus:outline-none ${
              darkMode
      ...
      {menuOpen && (
        <nav
          className={`md:hidden px-6 py-4 flex flex-col space-y-2 rounded-b-lg ${
            darkMode ? "bg-gray-800 text-gray-300" : "bg-white text-blue-800"
          }`}
          role="navigation"
